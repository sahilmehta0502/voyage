...
# ── Named crew pool ───────────────────────────────────────────────────
CREW_DATA = [
    # (first, last, role, license, phone, base)
...
def seed():
    ...
    with app.app_context():
        ...
        for i, (fn, ln, role, lic, ph, base) in enumerate(CREW_DATA):
            ...
            db.session.add(cm); crew_members.append(cm)
        db.session.flush()
        ...
        route_objs = []
        for rd in ROUTES:
            # duration: distance / 60 km/h + buffer
            dur = round(rd["distance_km"] / 60 + 1.5, 1)
            fare_child = round(rd["base_fare"] * 0.6, 0)
...
# Day 0 = today, gives past trips for analytics; days 1-14 = future for booking
        # Day 0 = today, gives past trips for analytics; days 1-14 = future for booking
        for day_offset in range(14):
            ...
            for route in route_objs:
                ...
                for dep_hour in DEPARTURE_HOURS:
                    ...
                    # Crew assignment by distance rule
                    dist = route.distance_km
                    if dist <= 750:
                        required = {"Driver": 1, "Co-Driver": 1, "Conductor": 1, "Attendant": 1}
                    elif dist <= 900:
                        required = {"Driver": 1, "Co-Driver": 1, "Conductor": 2, "Attendant": 1, "Guard": 1}
                    else:
                        required = {"Driver": 2, "Co-Driver": 1, "Conductor": 2, "Attendant": 2, "Guard": 1}

                    used_ids = set()
                    for role, count in required.items():
                        candidates = [c for c in crew_members
                                      if c.role == role and c.id not in used_ids
                                      and c.is_rested(dep_time)]
                        chosen = random.sample(candidates, min(count, len(candidates)))
                        for cm in chosen:
                            tc = TripCrew(trip_id=t.id, crew_member_id=cm.id, role_on_trip=role)
                            db.session.add(tc)
                            used_ids.add(cm.id)
        ...
        for fn, ln, em, ph in PASS_DATA:
            ...
            db.session.add(p); pass_objs.append(p)
        db.session.flush()

        # ── Bookings (past trips) ─────────────────────────────────────
        past_trips = [t for t in trips_created if t.departure_time < now][:60]
        booking_count = 0
        for trip in past_trips:
            n = random.randint(int(trip.seats_total * 0.4), int(trip.seats_total * 0.95))
            seats_used = set()
            ...
            while i < len(seats_list):
                ...
                adults = len(batch); children = 0
                if adults > 1 and random.random() < 0.3:
                    children = 1; adults -= 1
                fare = adults * trip.fare_adult + children * trip.fare_child
                ref = f"VYG{random.randint(100000,999999)}"
                b = Booking(booking_ref=ref, trip_id=trip.id, passenger_id=p.id,
                            passenger_name=p.full_name, passenger_email=p.email,
        ...
        print(f"\nSummary: {len(buses)} buses | {len(crew_members)} crew | "
              f"{len(route_objs)} routes | {len(trips_created)} trips | {booking_count} bookings")
